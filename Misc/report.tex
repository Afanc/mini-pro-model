...
\twocolumn[{
	...
	\centering
	    ...
	    \end{subfigure}
	    \caption{Bifurcation Diagram : \red{$X_{min}$} and \green{$X_{max}$} plotted at time intervals $[9/10; 1]$ of $t_{max}$, meaning it plots the maximum and minimum values in the last tenth of the simulation where we are sure that a either a limit cycle or a stable fixed point has already been reached. \\
...
\caption{In the above url you can find the collection of .gif files that we made to show the evolution of X in all N cells over time. \\
In C_2_gif_1 until C_2_gif_6 we see the evolution of the complexity of our system. The titles in the .gif files give the indication as to what part of the model was added. A is the reference to the correlation matrix between the cells. Lambda refers to the multiplier of the period. N is the number of cells simulated. A gradient of lambdas is introduced to emulate wave-like propagation of the signal in the SCN. In the last two heat maps, we simulate synchronisation between the two halves of the SCN, with no connection between the two at first and different base periods to see if they could synchronise if they were out of sync. In the second one there is a very strong connection between the two halves, and synchronisation takes place again.
